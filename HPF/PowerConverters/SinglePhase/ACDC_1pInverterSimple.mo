...
model ACDC_1pInverterSimple
  extends HPF.PowerConverters.SinglePhase.ACDC_1pConverterBase(phaseLN(
                                                               start_i_im =         cat(1, {-IAC_nom * sin(vAngle)}, {0.0 for i in 1:systemDef.numHrm - 1}), start_i_re =         cat(1, {-IAC_nom * cos(vAngle)}, {0.0 for i in 1:systemDef.numHrm - 1})));
  ...
  parameter Real gamma = 0.0 "Loss model quadratic term (per-unit)" annotation (
    Dialog(group = "Converter Loss Model"));
  parameter Modelica.Units.SI.Power P_stby=0 "Standby (night tare) loss"
