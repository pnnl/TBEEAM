...
model ACDC_ParameterizedMdl "AC to DC converter parameterized harmonic model"
  ...
  /* Input output power relation (Total input AC Power (sum over all harmonics))
      ...
      */
  Real P = HPF.PowerConverters.HelperFunctions.stbyPwrTransition(P_DCmin, P_stby, P_DC) * P_stby + (1 - HPF.PowerConverters.HelperFunctions.stbyPwrTransition(P_DCmin, P_stby, P_DC)) * (P_DC + alpha + beta * P_DC + gamma * P_DC ^ 2);
  /*
          Measurements
      */
