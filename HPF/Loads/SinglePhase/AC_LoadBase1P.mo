...
partial model AC_LoadBase1P
  ...
  parameter Modelica.Units.SI.ReactivePower Q_nom=0 "Nominal reactive power"
    annotation (Dialog(group="Device Ratings"));
  parameter Modelica.Units.SI.Voltage V_nom=120 "Nominal AC operating voltage"
    annotation (Dialog(group="Device Ratings"));
  ...
  final parameter Modelica.Units.SI.Current I_nom=Modelica.ComplexMath.abs(
      S_nom)/V_nom "Nominal AC current";

  // Initialization
  parameter Modelica.Units.SI.Angle vAngle_init=0
    "Nominal voltage angle for solver init"
    annotation (Dialog(group="Initialization"));
  final parameter Modelica.Units.SI.ComplexVoltage V_init=
      Modelica.ComplexMath.fromPolar(V_nom, vAngle_init);
  final parameter Modelica.Units.SI.ComplexCurrent I_init=
      Modelica.ComplexMath.conj(S_nom/V_init);
