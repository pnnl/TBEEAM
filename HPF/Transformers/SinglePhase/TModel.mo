...
model TModel "Transformer harmonic T-model"
  ...
  parameter Modelica.Units.SI.Voltage nomV_prim=280
    "Primary nominal voltage magnitude (at fundamental)";
  parameter Modelica.Units.SI.Voltage nomV_sec=120
    "Secondary nominal voltage magnitude (at fundamental)";
  ...
  parameter Modelica.Units.SI.Resistance Rp=1
    "Primary winding resistance (at fundamental)";
  ...
  HPF.SinglePhase.Components.IdealTransformer idealTransformer( N = nomV_prim / nomV_sec, start_v_im_prim = cat(1, {nomV_prim * sin(phi_prim)}, {1.0 for i in 1:systemDef.numHrm - 1}), start_v_im_sec = cat(1, {nomV_sec * sin(phi_sec)}, {1.0 for i in 1:systemDef.numHrm - 1}), start_v_re_prim = cat(1, {nomV_prim * cos(phi_prim)}, {1.0 for i in 1:systemDef.numHrm - 1}), start_v_re_sec = cat(1, {nomV_sec * cos(phi_sec)}, {1.0 for i in 1:systemDef.numHrm - 1})) annotation (
    Placement(evaluate=true, visible = true, transformation(origin = {26, -2}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
