...
model IdealTransformer
  extends HPF.SinglePhase.Interface.TwoPortBase(start_v_re_prim(start = {1.0 for i in 1:systemDef.numHrm}) = {1.0 for i in 1:systemDef.numHrm},start_v_re_sec(start = {1.0 for i in 1:systemDef.numHrm}) = {1.0 for i in 1:systemDef.numHrm});
