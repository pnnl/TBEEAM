...
package Interface
  ...
  partial model TwoPortBase
    ...
    */
    ...
    parameter Real start_v_re_sec[systemDef.numHrm] = {277.0 for i in 1:systemDef.numHrm} "Start value for voltage real part (secondary)";
    ...
    Complex vSec[systemDef.numHrm](re(start = start_v_re_sec, each fixed = false), im(start = start_v_im_sec, each fixed = false)) "Complex voltage secondary winding";
  ...
  partial model ACDC_ConverterBase
    ...
    parameter Modelica.Units.SI.Voltage nomV=120 "Nominal AC operating voltage"
      annotation (Dialog(group="Device Ratings"));
    final parameter Modelica.Units.SI.Current nomI=nomP/nomV
      "Nominal AC operating current";
    ...
    // Loss Model
    parameter Modelica.Units.SI.Power P_stby=0
      "Standby (no load) input AC power"
